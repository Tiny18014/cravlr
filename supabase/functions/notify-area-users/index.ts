...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `📍 Finding users for request. Coordinates: ${hasCoordinates ? "Yes" : "No"}. City: ${request.location_city}`,
    );
    ...
    const { data: potentialUsers, error: usersError } = await supabase
      .from("profiles")
      .select(
        "id, display_name, notify_recommender, recommender_paused, profile_lat, profile_lng, notification_radius_km, location_city, location_state",
      )
    ...
    const eligibleUsers = (potentialUsers || []).filter((u) => {
      ...
      if (hasCoordinates && u.profile_lat && u.profile_lng) {
        const distance = calculateDistance(request.lat, request.lng, u.profile_lat, u.profile_lng);
