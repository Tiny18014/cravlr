-- Add closed_at field to food_requests table if not exists
DO $$ 
BEGIN 
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.food_requests ADD COLUMN closed_at TIMESTAMP WITH TIME ZONE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 ...
                 AND column_name = 'location_lat') THEN
    ALTER TABLE public.profiles ADD COLUMN location_lat NUMERIC;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'profiles' 
                 AND column_name = 'location_lng') THEN
    ALTER TABLE public.profiles ADD COLUMN location_lng NUMERIC;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'profiles' 
                 AND column_name = 'points_total') THEN
    ALTER TABLE public.profiles ADD COLUMN points_total INTEGER DEFAULT 0 NOT NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'profiles' 
                 AND column_name = 'points_this_month') THEN
    ALTER TABLE public.profiles ADD COLUMN points_this_month INTEGER DEFAULT 0 NOT NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'profiles' 
                 AND column_name = 'notification_email') THEN
...
CREATE TABLE IF NOT EXISTS public.recommendations (
  ...
  UNIQUE(request_id, recommender_id)
);
...
DROP POLICY IF EXISTS "Users can view recommendations for requests they can see" ON public.recommendations;
...
CREATE INDEX IF NOT EXISTS idx_recommendations_request_id ON public.recommendations(request_id);
CREATE INDEX IF NOT EXISTS idx_recommendations_recommender_id ON public.recommendations(recommender_id);
CREATE INDEX IF NOT EXISTS idx_food_requests_status_expires ON public.food_requests(status, expires_at);
...
DROP TRIGGER IF EXISTS recommendations_slugify_trigger ON public.recommendations;
...
DROP TRIGGER IF EXISTS auto_close_request_trigger ON public.recommendations;
