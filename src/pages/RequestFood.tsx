...
const requestSchema = z.object({
  ...
  locationCity: z.string().trim().min(1, "City is required").max(100, "City name is too long"),
  // Region/State/Province - optional, free-form text for global support
  locationState: z.string().trim().max(150, "Region name is too long").optional().or(z.literal("")),
  ...
  countryCode: z
    ...
    .optional()
    .or(z.literal("")),
  locationAddress: z.string().trim().max(200, "Address is too long").optional(),
  additionalNotes: z.string().trim().max(500, "Additional notes must be less than 500 characters").optional(),
  responseWindow: z
    .number()
    .int()
...
const RequestFood = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const flavorPart =
        validated.flavorMood && validated.flavorMood !== "Anything" ? validated.flavorMood : "Any Flavor";
      const cuisinePart =
        validated.cuisineStyle && validated.cuisineStyle !== "Anything" ? validated.cuisineStyle : "Any Cuisine";
        ...
        <Card className="max-w-3xl mx-auto border-border/50">
          <CardHeader className="text-center pb-4">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Tell us your preferences and we'll find the best recommendations
            </p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-8">
              {/* Dish Type Section */}
              <DishTypeAutocomplete value={selectedDishType} onSelect={setSelectedDishType} />

              {/* Flavor Mood Section */}
              <FlavorMoodAutocomplete value={selectedFlavorMood} onSelect={setSelectedFlavorMood} />

              {/* Cuisine Style Section */}
              <CuisineAutocomplete value={selectedCuisine} onSelect={setSelectedCuisine} />
